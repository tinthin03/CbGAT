...
class Graph(core._MetaContainer):
    ...
    def __init__(self, edge_list=None, edge_weight=None, num_node=None, num_relation=None,
        ...
        #edge_list表示一系列实体id、关系id组成的三元组(h,r,t)或反向三元组的list
        edge_list = self._standarize_edge_list(edge_list, num_relation)#edge_list转为tensor
        edge_weight = self._standarize_edge_weight(edge_weight, edge_list)#与edge_list等长且同device的weight组成的一维tensor，默认为1
        num_edge = len(edge_list)#训练集的连接数量，一般fb15k-237为225xxx

        if num_node is None:
            num_node = self._maybe_num_node(edge_list)#从连接边来估计实体数量，会漏掉孤立实体.code写错了吧...
        if num_relation is None and edge_list.shape[1] > 2:
            num_relation = self._maybe_num_relation(edge_list)#从连接边来估计谓词数量
    ...
    def from_dense(cls, adjacency, node_feature=None, edge_feature=None):
        ...
        """
        adjacency = torch.as_tensor(adjacency)#shape=14541,14541的邻接矩阵，其值为r的id
        if adjacency.shape[0] != adjacency.shape[1]:
            raise ValueError("`adjacency` should be a square matrix, but found %d and %d" % adjacency.shape[:2])

        edge_list = adjacency.nonzero()#从邻接矩阵得到三元组h,t
        edge_weight = adjacency[tuple(edge_list.t())]#邻接矩阵的值为r的id
