...
namespace cppext_rotate_compare {
	...
	void forward_out(int64_t n, int64_t d,//n = pa.size(0)即当前评估器下各个规则判断的tail实体的总数量, d = a.size(1) / 2;即hOt的维度，区分实部、虚部
		const scalar_t *a, const scalar_t *b, 
		const int64_t* pa, const int64_t* pb,//#a,b,pa,pb分别是rule_embed, self.rotate.entity_embed, crule, centity
		scalar_t *dist) {

		INIT_VARS
		thn >>= 5;

		for(int64_t pos = (thi >> 5); pos < n; pos += thn) {//遍历当前评估器下各个规则判断的所有tail实体
			const scalar_t *are = a + (2 * d * pa[pos]);//pa（规则序号）的传入，仅仅是提供了存放指针，用来存放、指向各个规则的hOr返回值
			const scalar_t *bre = b + (2 * d * pb[pos]);//pb(当前评估器判断的tail的id)作用类似，提供存放、指向实体向量的指针
			const scalar_t *aim = are + d, *bim = bre + d;//are为该pgnd样本对应的规则的hOr，bre为pgnd样本对应的tail值，即centity
			...
			if((thi & 31) == 0) dist[pos] = value;//对当前评估器下各个规则判断的所有tail实体(数量即n，等于crule, centity的长度)，计算其hOt，与centity中实体向量的欧式距离相似度
		}
	...
	Tensor forward(const Tensor& a, const Tensor& b, const Tensor& pa, const Tensor& pb) {
		...
		Tensor dist = zeros({n}, a.options());//初始化一个长度为 n 的数组 dist，并将其所有元素都赋值为 0，使用的选项来自于 a 对象。
